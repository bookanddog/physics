...
/** 用鼠标旋转摄像机 */
void Camera::setViewByMouse(int w, int h)
{
	Vector3 mousePos;	///////////////////////////2d struct  change								  /**< 保存当前鼠标位置 */
	int middleX = w >> 1; /**< 得到屏幕宽度的一半 */
	int middleY = h >> 1; /**< 得到屏幕高度的一半 */
...
void Camera::yawCamera(float speed)
{
	...
	//** 进行边界检查和限定 *
	if(m_Position.x > (MAP_WIDTH / 2 ) || m_Position.x < -(MAP_WIDTH / 2 ))
	{
		m_Position.x = oldPos.x ;
		m_View.x = oldView.x;
	}

	if(m_Position.z > (MAP_WIDTH / 2 - CELL_WIDTH) || m_Position.z < -(MAP_WIDTH / 2 ))
	{
